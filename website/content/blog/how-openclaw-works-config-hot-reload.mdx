...
Post #1 in **How OpenClaw Works** — a series studying patterns from the [OpenClaw](https://github.com/openclaw/openclaw) codebase, an open-source personal AI assistant.

Source: [`config-reload.ts`](https://github.com/openclaw/openclaw/blob/9475791d/src/gateway/config-reload.ts)

**Key dependencies:**

- [chokidar](https://github.com/paulmillr/chokidar) — Node.js has built-in `fs.watch`, but it fires duplicate events, misses renames on Linux, and lacks recursive watching on older platforms. Chokidar normalizes these OS quirks and adds `awaitWriteFinish` to handle non-atomic writes — exactly what a config watcher needs.
...
Next in the series: [how OpenClaw's heartbeat runner turns a periodic timer into proactive behavior](/blog/how-openclaw-works-heartbeat-runner).
