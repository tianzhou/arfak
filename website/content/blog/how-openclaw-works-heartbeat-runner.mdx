...
Each agent tracks its own `nextDueMs`. The scheduler picks the earliest and sets a single timer. When a wake event arrives (exec completion, cron trigger), it can target a specific agent by ID or session key, advancing that agent's due time to now. Each tick calls into the agent loop ([Post #3](/blog/how-openclaw-works-agent-loop)), which serializes the run into the session queue — so a heartbeat tick never races a user message for the same session.
...
Next in the series: [how OpenClaw's agent loop turns a single LLM call into a resilient execution cycle](/blog/how-openclaw-works-agent-loop).
